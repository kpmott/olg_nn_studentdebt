...
losses = train(epochs=500,batchsize=32,lr=1e-5)
if saveTrain:
    model.eval()
    torch.save(model.state_dict(), params.savePath+'/.trained_model_params.pt')
    model.train()

losses = train(epochs=500,batchsize=64,lr=1e-6,losses=losses)
if saveTrain:
    model.eval()
    torch.save(model.state_dict(), params.savePath+'/.trained_model_params.pt')
    model.train()

losses = train(epochs=500,batchsize=100,lr=1e-7,losses=losses)
if saveTrain:
    model.eval()
    torch.save(model.state_dict(), params.savePath+'/.trained_model_params.pt')
    model.train()